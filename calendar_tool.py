...
def get_calendar_credentials() -> Credentials:
    ...
    # token.pickle saves your login after first time
    if os.path.exists("token.pickle"):
        with open("token.pickle", "rb") as token:
            credentials_calendar = pickle.load(token)

    # If no valid token, open browser login
    if not credentials_calendar or not credentials_calendar.valid:
        if credentials_calendar and credentials_calendar.expired and credentials_calendar.refresh_token:
            credentials_calendar.refresh(Request())   # silently refreshes if expired
        else:
            flow = InstalledAppFlow.from_client_secrets_file(
                os.getenv("GOOGLE_OAUTH_CLIENT_FILE"),  # path to your OAuth2 client JSON
                CALENDAR_SCOPES
            )
            credentials_calendar = flow.run_local_server(port=0)  # opens browser ONCE

        # Save token for next time
        with open("token.pickle", "wb") as token:
            pickle.dump(credentials_calendar, token)
